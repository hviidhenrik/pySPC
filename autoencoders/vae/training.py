    ...
    def train_model(self, model, patience=10, n_epochs=100, path='best_model.pt'):
        ...
        for epoch in range(1, n_epochs + 1):
            ...
            for batch, data in enumerate(self.train_loader, 1):
                optimizer.zero_grad()
                mean, variance = model.encode(data)
                encoded = model.reparameterize(mean, variance)
                decoded = model.decode(encoded)
                loss = model.loss_function(x_encoded=encoded, x_reconstructed=decoded, original=data, mu=mean, log_var=variance)
                loss.backward()
                optimizer.step()
